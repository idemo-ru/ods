---
title: 1.3.__id__. Определение  ~  __title__
description: IDEMO ~ Ontology of Dynamic Systems. __description__
published: true
date: 2025-12-09T16:57:30.838Z
tags: 
editor: markdown
dateCreated: 2025-12-09T16:52:08.586Z
---

## 1.3.__id__.1. Основное {#1-3-__id__-1}
Статус: | `Draft` | Candidate | Stable | Canonical |

*Версия: __doc_v_status_index_minor__ ∙ __месяц__ 2025*

> Документ описывает __одно_короткое_предложение__ суть определения
{.is-info}

## 1.3.__id__.2. Связанные документы {#1-3-__id__-2}
### 1.3.__id__.2.1. Исходящие *(ссылается этот документ)*
- [__1lvl__.__2lvl__.__id__. __form__~ __title__](__link__.md)
- [__1lvl__.__2lvl__.__id__. __form__~ __title__](__link__.md#__anchor__)
### 1.3.__id__.2.2. Входящие *(ссылаются на этот документ)*
- [__2lvl__.__2lvl__.__id__. __form__~ __title__](__link__.md)
- [__2lvl__.__2lvl__.__id__. __form__~ __title__](__link__.md#__anchor__)

## 1.3.__id__.3. Термин и формулировка {#1-3-__id__-3}
__content__

## 1.3.__id__.4. Классификация / Категория {#1-3-__id__-4}
__content__

## 1.3.__id__.5. Атрибуты / свойства {#1-3-__id__-5}
__content__

## 1.3.__id__.6. Область применения {#1-3-__id__-6}
__content__

## 1.3.__id__.7. Примеры и иллюстрации {#1-3-__id__-7}
__content__

## 1.3.__id__.8. Свободные блоки {#1-3-__id__-8}
__content__

## 1.3.__id__.9. Заключение {#1-3-__id__-9}
__content__

**Файл:** /ru/1-core/3-definitions/__slug__
