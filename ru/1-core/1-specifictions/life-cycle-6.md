...
**Фазовая аксиома** — жёсткая таблица Phase : DegradeVector → {birth, develop, climax, degrade, turn, end} (Прил. X).  
Наблюдатель **вычисляет** вектор и **читает** фазу; интерпретации **нет**.

> Примечание: если DegradeVector попадает в **неоднозначную ячейку**, система считается находящейся в **фазе wait** до тех пор, пока вектор не выйдет в однозначную зону.

## 1.1.1.5.1. ClassTag — идентификатор класса {#1-1-1-5-1}

ClassTag = `C-{число}-{краткое-описание}` (8 байт ASCII) связывает систему со строкой классификации 1…6 из [1.3.0.5.1](https://github.com/idemo-ru/ods/blob/main/ru/1-core/3-definitions/0-system.md#1-3-0-5-1).  
Tag выставляется при рождении и **не меняется** внутри одного LifeCycle; при transform сохраняется, если **базовая идентичность** подтверждена, иначе присваивается **новый** Tag.
