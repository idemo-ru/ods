...
## tmp.1.1.6.1.1. Упрощённый ChangeFlow для детерминированных систем {#tmp-1-2-6-1-1}

Для систем классов 0–1 (детерминированные / объектные) полный ChangeFlow-6 сокращается до строго локальной фазы implement(). Все остальные фазы (collect, analyze, forecast, decide, evaluate) выполняются либо родительской системой, либо субъектом, инициирующим ChangeFlow.
...
```
Примеры ожидания approve от агента user:
```py
$flow_name(
	@(
  	context_path:data
