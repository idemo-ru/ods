...
- безусловным:
```=> [] ^(next decide phase)``` 
	короткий синтаксис безусловного ```=> ^(next decide phase)```. Безусловный подразумевает естественное завершение предыдущей фазы (например завершение вычислений алгоритма).
