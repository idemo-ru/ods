...
| `@`    | context        | Контекст            |                         |
| `=>`    | phase_transition      | Оператор фазового перехода LC-6, CF-6. Может быть условным ```=> [cond] ?(next collect phase)``` и безусловным - ```=> [] ^(next decide phase)``` короткий синтаксис безцсловного ```=> ^(next decide phase)```. Безусловный подразумевает естественное завершение предыдущей фазы (например завершение вычислений алгоритма)                            |                        |
