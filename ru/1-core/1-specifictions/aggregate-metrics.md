...
> Алгоритм выхода из `wait`: наблюдатель **ежед Δt_obs** пересчитывает DegradeVector; как только вектор попадает в **однозначную ячейку**, фаза фиксируется и **wait снимается**.
