...
## 2.2.11.6. 
### 2.2.11.6.1. Связь V (вариативность свободы воли) с теорией сложности
1. Высокая V → рост вычислительной сложности (P → NP-подобные ветвления в forecast/decide).
2. Мета-процессы → экспоненциальный поиск (irreducibility по Wolfram).
3. Ограничения наблюдателя (конечные ресурсы) → полиномиальная аппроксимация (coarse-graining снижает сложность).
4. V как параметр сложности: низкая V — P (детерминизм); высокая — NP-hard (множество альтернатив).
V увеличивает сложность для адаптации.

### 2.2.11.6.2. Связь мета-процессов с теорией информации:
Мета-процессы снижают энтропию системы (H) через coarse-graining и выбор альтернатив (реюз опыта → сжатие неопределённости).
Генерация/оценка альтернатив → рост информации (I = -log P).
Рекурсия → обработка мета-информации (information about information).
Ограниченность ресурсов → Landauer limit на стирание альтернатив.

Мета-процессы — механизм обработки и снижения информационной энтропии для адаптации.
